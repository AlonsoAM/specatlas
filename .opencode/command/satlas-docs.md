---
description: Genera la documentación técnica y manual del cambio (carril completo) desde la evidencia registrada. Usar cuando el usuario pide documentar, generar el manual o cerrar la documentación antes de archivar.
---
...
Dejar la documentación técnica y manual del cambio `$ARGUMENTS` en `.sdd/changes/$ARGUMENTS/docs/`, construida desde lo real (especificación, tareas y evidencia).
El contenido se escribe en **español**.

## Pasos

1. Ejecuta `satlas docs $ARGUMENTS` para generar el esqueleto de los dos documentos desde las plantillas y la evidencia.
...
5. Cierra con `satlas docs $ARGUMENTS` (regenera el bloque gestionado) y `satlas next $ARGUMENTS`.
