...
| Open VSX | Namespace `specatlas` | `OVSX_PAT` |
...
pnpm --filter specatlas-vscode exec vsce publish -p "$VSCE_PAT"
pnpm --filter specatlas-vscode exec ovsx publish -p "$OVSX_PAT"

# 6. Etiquetar
git tag v0.1.0 && git push --tags
```

El workflow `.github/workflows/release.yml` ejecuta los pasos 1, 4 y 5 automáticamente al empujar una etiqueta `v*` (omite los pasos cuyo token no esté configurado).
...
- `packages/vscode/README.md` no admite SVG (restricción de `vsce`); usa `media/logo.png`.
